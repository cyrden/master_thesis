    ...
    while (1) {
        ...
        case EBPF_OP_DIV_REG:
            if (reg[inst.src] == 0) {
                ...
                zlog_notice("exec: division by 0 !!!");
                return UINT64_MAX;
            }
        ...
        case EBPF_OP_MOD_REG:
            if (reg[inst.src] == 0) {
                fprintf(stderr, "uBPF error: division by zero at PC %u\n", cur_pc);
                zlog_notice("exec: division by 0 at PC !!!");
                return UINT64_MAX;
            }
            reg[inst.dst] = u32(reg[inst.dst]) % u32(reg[inst.src]);
            break;
...
validate(const struct ubpf_vm *vm, const struct ebpf_inst *insts, uint32_t num_insts, char **errmsg)
{
    if (num_insts >= MAX_INSTS) {
        *errmsg = ubpf_error("too many instructions (max %u)", MAX_INSTS);
        zlog_notice("validate: Too many instructions !!!!! \n");
        return false;
    }
    ...
    if (exit_insts_index >= num_insts) {
        *errmsg = ubpf_error("no exit in instructions");
        zlog_notice("validate: No exit in instructions !!!!! \n");
        return false;
    }
        ...
        case EBPF_OP_BE:
            if (inst.imm != 16 && inst.imm != 32 && inst.imm != 64) {
                *errmsg = ubpf_error("invalid endian immediate at PC %d", i);
                zlog_notice("validate: invalid endian immediate at PC !!!!! \n");
                return false;
            }
            break;
        ...
        case EBPF_OP_LDDW:
            if (i + 1 >= num_insts || insts[i+1].opcode != 0) {
                *errmsg = ubpf_error("incomplete lddw at PC %d", i);
                zlog_notice("validate: incomplete lddw at PC !!!");
                return false;
            }
        ...
        case EBPF_OP_CALL:
            if (inst.imm < 0 || inst.imm >= MAX_EXT_FUNCS) {
                *errmsg = ubpf_error("invalid call immediate at PC %d", i);
                zlog_notice("validate: invalid call immediate at PC !!!");
                return false;
            }
            if (!vm->ext_funcs[inst.imm]) {
                *errmsg = ubpf_error("call to nonexistent function %u at PC %d", inst.imm, i);
                zlog_notice("validate: call to non existing function !!!!! \n");
        ...
        case EBPF_OP_MOD64_IMM:
            if (inst.imm == 0) {
                *errmsg = ubpf_error("division by zero at PC %d", i);
                zlog_notice("validate: division by 0 at PC !!!");
                return false;
            }
            break;
